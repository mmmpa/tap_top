...
import ChartConfiguration from './pretty-simple-chart/models/chart-configuration';
...
class Watcher {
  constructor ({
    dataLength, wait, tasks, totalCPU, totalMemory,
  ...
  initialize () {
    ...
    const base = new ChartConfiguration({
      dataLength: Configure.size,
      xLabels: this.timeList,
    });

    const totalCPU = new PrettySimpleChart('totalCPU', base.cloneWith({
      name: 'totalCPU',
      data: CPUList,
      isStackChart: true,
    })).render();

    const totalMemory = new PrettySimpleChart('totalMemory', base.cloneWith({
      name: 'totalMemory',
      data: memoryList,
      isStackChart: true,
    })).render();

    const CPUGraph = new PrettySimpleChart('CPUGraph', base.cloneWith({
      name: 'PerCPU',
      data: pickRow('%CPU', processList),
      isStackChart: false,
    })).render();

    const memoryGraph = new PrettySimpleChart('memoryGraph', base.cloneWith({
      name: 'PerMemory',
