...
export default class ChartComponent extends React.Component {
  ...
  get rowItems () {
    ...
    for (let i in this.state.data.data) {
      ...
      items.push(<RowItem {...{ value, data, conf }}/>)
    }
...
class RowItemContainer {
  render () {
    let { w, h, yLabelWidth, xLabelHeight, rowItemWidth } = this.props.conf
    return <div className="ps-chart ps-row-item-container" style={{
      top: 0,
      left: w + xLabelHeight,
      width: rowItemWidth,
...
class RowItem {
  render () {
    let { name, color } = this.props.data
    let { value } = this.props
    return <div className="ps-chart ps-row-item">
      <div className="ps-chart ps-row-item-color" style={{
        color,
...
class Line {
  ...
  render () {
    ...
    return (
      <g id={this.props.data.name + this.props.data.id} {...{ onMouseOver }} key={this.props.data.name}>
        <path fill={stack ? this.props.data.color : 'none' } d={this.lineValue} style={`stroke:${this.props.data.color};stroke-width:1`}/>
      </g>)
  }
}